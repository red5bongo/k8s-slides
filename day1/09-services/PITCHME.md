...
A service is an abstraction for pods, providing a stable, virtual IP (VIP)
address.
...
not an actual IP address connected to a network interface but its purpose is purely to forward traffic to one or more pods. 
